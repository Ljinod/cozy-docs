...
* **repID**: the replication id generated by CouchDB. To share data we use the
  replication feature provided by CouchDB. This field is only useful when then
  *sync* field is set to true, we will tell more on this later.
...
if we want to manage/monitor it, in case the replication is synchronous. Indeed
if the replication is not synchronous then the replication process is a one
