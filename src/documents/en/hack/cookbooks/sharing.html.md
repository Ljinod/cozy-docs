...
access it.<br />
To sum up: CouchDB generates the **id** and Cozy-sharing automatically adds the
**docType**. We found the first two items on our list.
...
Once the recipient's Cozy has received the request, it will also create a
sharing document copying all the information it received. The id is also kept
but it will be used as a login, again we let CouchDB generates the id of newly
created documents to avoid any collision.

> *Bob*: If I understand correctly  you're creating a document on my Cozy even
> if, in the end, I decline?<br />
> *Cozy*: Well yes, if we don't store it and only keep it in RAM then, if you
> decide to reboot your Cozy you would loose the request in the process.<br />
...
The second scenario is a bit more complicated: in order to do things properly
we have to operate a certain way.


### Accepting a request: the ways of access control

As you may have guessed if someone shares some document with you it means (s)he
will have an access on your Cozy. Thus it is important that with this access
(s)he can only do what (s)he is supposed to and nothing more.
...
the transaction on the recipient's Cozy. To memorize and later on actually
check those, an *Access* document is created:

```javascript
var Access = {
    id: "9ol;pm:^ù", // again, generated by CouchDB
...
sharer's and initiate the *replication*.
